...
test('standalone Pages route works as a mobile Smart Cards app', {timeout:20000}, async t => {
  ...
  try{
    ...
    assert.deepEqual(result.initial,{path:'/index.html',search:'?view=smart-cards',title:'WYZE Smart Cards',standalone:true,light:true,cards:5,hiddenChrome:true,brokenImages:[],cardWidth:366,sceneHeight:275});
    assert.deepEqual(result.alertStates,['PERSON','OPEN','Package left','NOT OUT','CARDINAL']);
    assert.deepEqual(result.expanded,{card:true,aria:'true',details:'grid',icons:2,feedbackButtons:10,videoItems:3,memoryItems:2,cardFeedback:'none',heading:'Supporting Evidence',preview:'none',cardHeight:640,sceneHeight:275,stateOffset:-12,goalVisibility:'hidden',hasVideo:true,hasMemory:true});
